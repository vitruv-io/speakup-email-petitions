...
function dk_speakup_petitions_page() {
	...
	$addnew_url = esc_url( site_url() . '/wp-admin/admin.php?page=dk_speakup_addnew' );

	switch ( $action ) {

		case 'delete' :
			// security: ensure user has intention
			check_admin_referer( 'dk_speakup-delete_petition' . $id );
			...
			$message_update = __( 'Petition deleted.', 'dk_speakup' );
